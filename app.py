...
def show_debug_info(video_path, processed_frames):
    ...
    # tab1, tab2, tab3 = st.tabs(["Video Info", "Frame Analysis", "Frames"])
    ...
    #                 with cols[j]:
    #                     img = Image.fromarray((processed_frames[idx] * 255).astype(np.uint8))
    #                     st.image(img, caption=f"Frame {idx}", use_column_width=True)
    ...
    if uploaded_file:
        try:
            ...
            with st.spinner("⏳ Processing..."):
                ...
                #     st.write(f"📦 Your Input Shape: {X.shape}")
